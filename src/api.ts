...
class BigQueryAPIClient implements BigQueryAPI {
  ...
  constructor(
    ...
    this.resourcesUrl = `/api/datasources/uid/${datasourceUid}/resources`;
  }
  ...
  getProjects = async (): Promise<GCPProject[]> => {
    return this.fromCache('projects', this._getProjects)(this.datasourceUid);
  };
  ...
  validateQuery = async (query: BigQueryQueryNG, range?: TimeRange): Promise<ValidationResults> => {
    const rawSql = getTemplateSrv()
      .replace(
        ...
        // Moreover, the interval and the rest of the query options are not passed down to the QueryEditor are these are dependant on
        // dashboard settings, panel size etc. Hardcoding the interval value here should not influence the query validation whatsoever.
        {
          __interval: {
            text: '1m',
            value: '1m',
...
const apis: Map<string, BigQueryAPI> = new Map();
