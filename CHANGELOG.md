...
🐛 Fix: `$__timeFrom()` and `$__timeTo()` called without a column now return the dashboard time boundary as a timestamp value (e.g. `TIMESTAMP('2024-01-01T00:00:00Z')`), matching the documented behavior, instead of generating invalid SQL such as `>= '2024-01-01T00:00:00Z'`. The filter forms `$__timeFrom(column)` and `$__timeTo(column)` are unchanged.
...
⚙️ Updated frontend & backend dependencies, resolving CVEs in `react-router` and related packages
